...
**Resolved 2026-07-26 (C-6.15):** hairline fade + cross-page tab position are fixed and confirmed, not just curl-checked this time — used `agent-browser` to actually screenshot the rendered page and sample pixel colors along the hairline. Root cause of the whole C-6.6–C-6.14 non-convergence: the fade stop was a `%` of the bar's own (viewport-dependent) width, which can never reliably line up with the `pl-96` reserved space's fixed pixel width. Switched `TopNavLine`'s mask stop to a fixed `24rem` (matching `pl-96` exactly) when a leading space is reserved, `1rem` otherwise — pixel-sampled confirmation: fully solid by local x=384px, first tab starts at x≈388px, holds at any viewport width by construction. Separately, homepage's desktop nav wrapper used `left-4` while the master booking page used `pl-28 sm:pl-32` — this ~112px mismatch was the cause of tabs visibly jumping when navigating between the homepage and any master/content page; unified to `pl-28 sm:pl-32` everywhere. `PageRenderer` (used by both content-page routes) now uses the exact same absolute nav-bar structure (reserved leading space + `actions` slot) as the homepage/master page instead of its own ad-hoc bare version — content pages' nav now looks identical to the rest of the site.

1. **Tab styling is explicitly rejected as-is** ("уебищные кнопки" — user's own words). The `rounded-full border` chip treatment from C-6.6/C-6.7 needs a real redesign, not another parameter tweak. The user has no fixed vision for this yet and said so directly — needs a proper design conversation (possibly the visual companion, or a few concrete mockup options to react to) rather than more one-shot CSS edits. **Added 2026-07-26:** user also wants the tabs' horizontal *position* nudged left from where they currently sit (`pl-96` reserved leading space) — explicitly deferred together with the style redesign rather than tuned as a one-off, since both will change together when the tabs are redone.
2. **New idea raised, unexplored:** tabs should feel visually related/"attracted" to the logo when one is configured — logo as an anchor the tabs cluster toward, rather than tabs sitting in an independent flat row. Vague on purpose (user's own words: "не знаю, короче") — needs to be workshopped, not assumed. Still open — not addressed by item 3's resolution below.
3. ~~Master booking page has no logo, so whatever "tabs relate to the logo" concept means needs a distinct, deliberate answer for master pages specifically.~~ — **Resolved/approved 2026-07-26.** User explicitly reviewed the master booking page nav live and approved it as-is: the fade lands right at the back button, the back button no longer collides with a logo, and the user is fine with the current floating/anchorless look even without a logo configured ("не факт, что там будут и логотип размещать, короче, похуй"). No further work needed here unless revisited later.
4. ~~Master booking page's calendar card has an unused gap below it — shrink the calendar, remove the dead gap.~~ — **Addressed 2026-07-26.** Removed the "select a service first" empty-state hint from `SlotsList.tsx` (was adding a row of dead space inside the calendar card) and tightened `[masterId]/page.tsx`'s `mt-8` → `mt-4` above the calendar/service cards, pulling the whole booking block closer to the "Забронювати візит" title. Also found and fixed a real, separate bug the user spotted live: a 44px empty gap between the calendar's month/nav row and the weekday row, caused by a `react-day-picker` v8→v9 migration leftover — `globals.css` hid the old v8 `.rdp-caption` class to make room for the custom month header, but v9 renamed that wrapper to `.rdp-month_caption`, so the untargeted wrapper kept reserving its height even with its (hidden) children collapsed. Added the correct `.rdp-month_caption { display: none !important }` rule alongside the old one. **Not fully closed on purpose:** the user wants the freed vertical space below the cards kept empty for now — it's reserved for a future photo gallery on this page, not something to fill immediately.
5. ~~Public content-page rendering itself (`/pages/[slug]`) is rough/unfinished visually.~~ — **Resolved 2026-07-26.** After the nav-bar consistency fix (C-6.15) the user reviewed live and said the page is basically fine ("он в принципе-то нормальный") — the one real complaint was the on-page `<h1>{title}</h1>` duplicating the page name already shown as the active nav tab. Removed from `PageRenderer.tsx` (along with the now-unused `page` prop/type and localized-title plumbing); content pages now render nav + blocks only, no redundant heading. Also fixed a real bug the user caught live: both content-page routes forced `min-h-screen` on their `<main>`, stacking on top of the root layout's own `min-h-screen flex flex-col` wrapper and pushing the footer off-screen (forced scrollbar) even on an empty page. Switched both routes to `flex-1`, matching the master booking page's pattern — verified via `agent-browser` that `document.documentElement.scrollHeight` now exactly equals `window.innerHeight` on an empty page.
