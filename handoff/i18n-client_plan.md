...
- [ ] A1: `src/components/booking-management/api/bookingManagementApi.ts`
...
- [ ] A2: `src/components/booking-management/hooks/useBookingMutations.ts`
...
- [ ] A3: `src/components/booking-management/hooks/useBookingHandlers.ts`
...
- [ ] A4: `src/components/booking-management/PanelRenderer.tsx`
  - L411 `?? 'Wystąpił nieznany błąd.'` → `?? t('errors.generic')` (wire `t`).
- [ ] A5: `src/components/booking-management/ContactMasterPanel.tsx`
...
- [ ] A6: `src/components/booking-management/EditProcedurePanel.tsx`
...
- [ ] A7: Verify remaining booking-management panels render only via `t()`
...
- [ ] B1: Run `node scripts/i18n-check.mjs` (from Phase 0) to get the authoritative list
  of referenced-but-missing keys, scoped to client files.
- [ ] B2: Auth components — add every missing key to all 3 files
...
- [ ] B3: Profile pages — add missing keys
...
- [ ] B4: Support + booking form — add missing keys
  - Files: `src/locales/*`; sources `src/app/support/page.tsx` (5),
    `src/components/BookingForm.tsx` (15).
- [ ] B5: Confirm the "already-existing key" two-arg calls need NO change
...
- [ ] C1: `src/components/ThemeToggle.tsx`
...
- [ ] C2: `src/components/ErrorBoundary.tsx` (class component — AD-5)
...
- [ ] C5: `src/components/data-export/ExportResultView.tsx`
  - Replace hardcoded Polish JSX (L31–61) with `t('gdpr.export.*')` (keys exist);
    it already has `t`. Add any missing sub-keys.
- [ ] C6: `src/components/data-export/exportFormat.ts` (downloadable CSV/JSON)
...
- [ ] D1: Update every caller of `client-validators.ts` to render `t(result.error)`
...
- [ ] E1: Landing `src/app/page.tsx` + `src/components/home/HomeClient.tsx` +
  `MasterSelector.tsx` — confirm full `t()` coverage; fix any inline literals.
- [ ] E2: Auth page wrappers `src/app/auth/{login,register,reset-password,forgot-password}/page.tsx`
...
- [ ] E3: `src/app/support/page.tsx`, `src/app/profile/page.tsx`, `profile/edit/page.tsx`
...
- [ ] F1: `node scripts/i18n-check.mjs` → zero referenced-but-missing keys in client files;
  identical key sets across pl/en/uk.
- [ ] F2: `npm run lint` (zero warnings) + `npm run test`.
- [ ] F3: Add/extend tests: a unit test asserting `apiErrorKey` covers every enumerated
...
## Acceptance Criteria
- [ ] `i18n-check` passes (no missing keys, identical key sets) for all client files.
- [ ] No hardcoded user-facing Polish/Russian literal remains in in-scope client files
      ...
      currency `zł`, comments).
- [ ] All booking-management API errors display translated text driven by `code`;
      switching language changes the error text.
- [ ] Auth register/login/reset forms render fully in pl and uk (no English leakage).
- [ ] Dates in client UI render in the active language's locale (AD-3).
- [ ] `npm run lint` + `npm run test` pass; DOX pass done for touched dirs.
