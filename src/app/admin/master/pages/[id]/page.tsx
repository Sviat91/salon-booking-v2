...
interface MasterPageDetailProps {
  params: { id: string }
}

export default async function MasterPageDetail({ params }: MasterPageDetailProps) {
  const t = getServerT()
  ...
  if (!page || page.ownerType !== "master" || page.masterId !== session.user.id) notFound()
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t('admin.pages.publicUrlLabel')}: <code className="text-foreground">/{session.user.id}/pages/{page.slug}</code>
        </p>
      </div>
