...
- `errors/apiErrorKey.ts` maps an API route's `{ code }` field to an `errors.*` i18n key (`apiErrorKey(code?) => 'errors.<code>' | 'errors.generic'`) — pure, no React, whitelist-based. Used to translate `{ error, code }` API responses at the React edge instead of displaying the raw (mixed-language) `error` text.
