...
- `localized-content.ts` is the framework-free resolver for per-locale DB columns (`Service.name_pl/en/uk`, `MasterProfile.bio_pl/en/uk`) — `resolveLocalized(field, lang)` picks `field[lang]`, falling back to `pl` then any non-empty variant; `parseEnabledLocales(json)` validates `TenantConfig.enabledLocales`. Client components resolve at render with `useCurrentLanguage()`; server components pass the full `{ pl, en, uk }` object down rather than pre-resolving.
