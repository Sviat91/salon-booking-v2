...
- `hooks/useTurnstileSession.ts` gates mutating actions (cancel, time/procedure change, and `ContactMasterPanel` → `POST /api/master/contact`) behind Cloudflare Turnstile — don't bypass it when adding a new mutating panel. A panel that posts to a public endpoint owns its **own** `useTurnstileSession` instance (e.g. `ContactMasterPanel.tsx`) rather than threading props through `PanelRenderer`/`BookingManagement`'s instance — panels are mutually exclusive so two instances are never mounted at once.
